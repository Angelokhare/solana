...
const ALCHEMY_CONFIG = {
  policyId: '220ba52a-47c4-4866-9144-0453e50016f7',
  apiKey: '_1jLjiAQQ5GUsJJHQ2Ivw',
  rpcUrl: 'https://solana-mainnet.g.alchemy.com/v2/_1jLjiAQQ5GUsJJHQ2Ivw',
  enabled: true
};
...
const Dashboard = () => {
  ...
  const [tokenType, setTokenType] = useState<'SOL' | 'SPL' | 'MULTI'>('SOL');
  ...
  // Add this after your other state declarations
    const [senderPaysATA, setSenderPaysATA] = useState(false);
  ...
  const fetchTokenAccounts = async () => {
    ...
    try {
      ...
      setTokenAccounts(filteredTokens);
  ...
  const handleMultiTokenSend = async () => {
    ...
    try {
      ...
      for (let tokenIdx = 0; tokenIdx < selectedTokens.length; tokenIdx++) {
        ...
        if (ataTxs.length > 0) {
          const useAlchemy = ALCHEMY_CONFIG.enabled && !senderPaysATA;
      ...
      setStatus('SUCCESS');
      const paymentInfo = senderPaysATA ? 'You paid' : (ALCHEMY_CONFIG.enabled ? 'FREE' : '');
      setStatusMsg(`✅ Sent ${selectedTokens.length} tokens to ${validRecipients.length} wallets! (${totalAtasCreated} ATAs created ${paymentInfo}, ${totalAtasExisted} existed)`);
    } catch (err: any) {
      setStatus('ERROR');
  ...
  return (
    <div className={`min-h-screen ${THEME.bg} ${THEME.text} font-sans`}>
      ...
      <label className="flex items-start gap-3 cursor-pointer">
        <input 
          type="checkbox" 
          checked={senderPaysATA}
          onChange={(e) => setSenderPaysATA(e.target.checked)}
          className="accent-blue-600 mt-1 w-5 h-5"
        />
        <div className="flex-1">
          <span className="font-bold text-blue-900 text-sm block">💰 I will pay for ATA creation fees</span>
          <span className="text-xs text-blue-800 mt-1 block">
            {senderPaysATA ? 
              "You'll pay ~0.00203 SOL per new ATA. Uncheck to use Alchemy sponsorship (if enabled)." :
              "Alchemy will sponsor ATA creation fees (if enabled). Check this box to pay yourself instead."
            }
          </span>
        </div>
      </label>
        ...
        <section className={THEME.card}>
          ...
          {tokenType === 'MULTI' ? (
            <div className="space-y-4 mb-6">
              ...
              {ALCHEMY_CONFIG.enabled && !senderPaysATA && (
                  <p className="text-xs text-green-600 font-bold">+ Alchemy pays all ATA creation costs!</p>
                )}
                {senderPaysATA && (
                  <p className="text-xs text-blue-600 font-bold">+ You will pay ATA creation fees (~0.00203 SOL each)</p>
                )}
            </div>
          ) : (
